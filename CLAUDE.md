...
What does work is grading from the master: per image, measure the 1st and 99th luminance percentile, map them 85% of the way toward a 20/247 black and white point with one LUT applied to all three channels, then a mild unsharp mask on the soft ones. The identical LUT per channel is what keeps hue intact. Verified by sampling the carmine pixels of the original and comparing the same coordinates after: hue moves 1.8 to 3.0 degrees on average, below the threshold where the eye reads a colour change, while saturation rises 10 to 18 percent, which is exactly the "less dull" the client asked for. **Measure hue as a circular mean over the same pixel set**; averaging raw hue angles across the 0/360 wrap, or re-detecting red pixels after the change, produces nonsense values in the 40 to 95 degree range.
