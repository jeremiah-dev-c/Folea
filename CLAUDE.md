...
**The product photo lives beside the form, not above it.** As its own block at the top of the page it was decoration with no argument for being there; in the form's right-hand column it is the thing the questions are about. It has no fixed ratio: the frame takes `h-full` and fills the grid row, so it always ends exactly level with the form card (measured 526px against 526px) even if a field is added later. Width stays capped at `21rem` so it does not overpower the text. **It is `hidden lg:block`**, because below `lg` there is no second column and the photo became a long block you had to scroll past on a phone. Note the paired `sizes="(min-width: 1024px) 38vw, 0px"`: with the `0px` branch the browser skips the download entirely on phones instead of fetching an image it will never paint. There is no caption under it any more.
...
**The form panel is the same `blush` as the section, set apart by a border.** Cream was tried first (too much beige on the page) and then ink with white type (didn't fit the rest); the client landed on a bordered card on 16 Aug. The border is what does the separating now, and a rectangle is the point: a lone horizontal line above the fields was read as an empty input, which is what started this whole sequence. Don't go back to a rule.

Every tone is tuned to blush, and blush is a light ground, so the muted values run darker than you would guess. Measured against `#f9a3bd`: labels and input text at full `ink` give 10.4, the placeholder needs `ink/65` to clear 4.5 (at `ink/55` it sat at 3.62), and the field underlines plus the card border need `ink/50` to clear the 3.0 that non-text indicators want (`ink/25` was 1.69 and barely visible). The submit button stays `ink` with white type; on the earlier black panel it had to flip to `blush`, so it moves with the panel colour.
