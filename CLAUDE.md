...
**The six product stills are all 4:5 (1920x2400) and cropped tight from the masters.** The client said the product photos looked weak, and the measurable cause was framing, not the photography: in the master files the jar filled only 8% of the frame on `product-front`, 12% on `product-top`, 15% on `product-trio` and 19% on `product-duo`. The web versions inherited that dead space. Since the masters are 3264x4896, cropping in hard still leaves ~1900px of width, so this cost nothing in resolution and needed no AI.

**Lighting was the second, separate complaint, and generative relighting is ruled out.** Measured tonal range on the shipped stills was 114 to 162 out of 255, with no true black and no clean highlight: that flatness is what reads as dull. One Higgsfield attempt (`seedream_v4_5`, 1 credit) proved the approach unusable here. Every image model in the catalogue takes the photo as `image_references`, not as a canvas to edit, so it re-shot the scene: the jar moved and shrank, the matte lid turned glossy, a hard cast shadow appeared, and the carmine drifted 33 points across RGB. The client's constraints (product unchanged, angle unchanged, no invented shadows, exact logo colour) cannot all hold while a model redraws the frame. Don't retry this with a different prompt; the failure is structural.

What does work is grading from the master, but **only upward**. The first attempt pulled the black point down to widen the range, and the client caught it immediately: in these high-key frames the darkest pixels *are* the printed label, so lowering the black point literally darkens the logo and the type. Measure the print first (it sits at luminance 73 to 99 here, against a 190 to 215 background), then use a curve that is identity below ~130 and lifts only above it, taking the white point to 250. That leaves the print untouched while the backdrop and the white lid clean up, so the carmine gains contrast against its surroundings instead of losing weight. Verified per image on the same pixel coordinates: print luminance moves 0.6 to 3.7 points and hue 0.4 to 0.6 degrees.

Keep unsharp masking mild and raise its threshold to about 6. At a low threshold it sharpens the edges of the printed type and adds dark halos, which reads as the same "the text got darker" problem by another route.

`product-butter` and `product-top` are excluded from the grading pass. The client calls it perfect and it is the only still with a real background falloff (measured 88 against 14 on `product-top`), which is why it reads better than the rest. There is no from-above frame of the jar in the shoot beyond `product-top`, so a new angle can only come from a reshoot.

The crop pass is reproducible: find the product's bounding box by thresholding brightness (white jar against grey seamless) plus the red-minus-blue channel (pink label and carmine type), pad it, force the box to 4:5, then encode from the master with `cjpeg -quality 84 -optimize -progressive`. **Detect the box with a generous threshold**: a strict one clips the jars' soft edges, and the first pass cut a jar in half at the frame edge. `product-trio` additionally needs its right edge clamped into the gap between the second and third jar, otherwise a sliver of the third jar hangs in frame.

Because every still is now exactly 4:5, the PDP gallery's `object-contain` no longer crops anything away; it was there because `product-trio` used to be 3:2 landscape while the rest were 2:3. The frame itself (`aspect-[4/5]` on mobile, `sm:h-[560px]` with a flex width above it) still letterboxes at mid widths, so it is worth revisiting if you touch that component. Check the aspect ratio of the file before choosing any new image frame.
