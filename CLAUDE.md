...
**The product photo lives beside the form, not above it.** As its own block at the top of the page it was decoration with no argument for being there; in the form's right-hand column it is the thing the questions are about. From `lg` up it drops its fixed 2:3 ratio and takes `h-full` so it ends exactly level with the form card (measured 526px against 526px), with its width still capped at `21rem` so it does not overpower the text. Below `lg` it sits under the form, keeps 2:3 and is capped at `max-w-sm`, since a full-width portrait there became a 750px wall. There is no caption under it any more.
