...
| `/producten/hairbutter` | PDP: fotogalerij met 5 beelden in een 2:3 kader met `object-cover`, prijs, aantal-selector, "in winkelmand", hoofdingrediënten en de gebruiksaanwijzing met twee methodes. **Geen sterren-rating, geen opsommingen.** |
...
| `/contact` | Roze opening (kop, mailadres groot, bereikbaarheid), roze formulier met onderstreepte velden en een keuzelijst voor het onderwerp, crème FAQ-verwijzing. Formulier is front-end only |
...
- **Contact**: opent in het roze en loopt door tot de crème FAQ-verwijzing, daarna de zwarte footer. De opening is kort gehouden (kop plus het mailadres groot ernaast) zodat het formulier hoog op de pagina begint. Het formulier staat op roze en is bewust rustig: labels boven onderstreepte velden, naam en e-mail naast elkaar, het onderwerp als keuzelijst. De eerdere invulbrief met chips vond de klant kinderlijk en is eruit. De productfoto staat ernaast in plaats van los bovenaan. De FAQ-verwijzing is beige, want zwart op zwart liep visueel over in de footer. De kopregel met haarlijn is er op 15 aug uitgehaald. **Er zit nog geen mailservice achter**, zie de openstaande punten.
...
- **Footer**: zwart, met de nieuwsbriefinschrijving bovenin en compacte linkkolommen (twee kolommen op mobiel). Onderin staan de copyright met KvK/BTW en de credit "Webdesign by SOM" met link naar snelonlinemarketing.nl. Het jaartal loopt automatisch mee: zie CLAUDE.md voor waarom dat via `useSyncExternalStore` gaat en niet via `new Date()` in de render.
...
- `public/images/product-*.jpg` (6 productstills) en `model-*.jpg` (16 modelfoto's), allemaal 2:3 staand op 1600x2400. `product-trio` was liggend en is op 15 aug naar staand gesneden, zodat de PDP-galerij zonder balken kan. De klant vond de productstills dof; er zijn drie verbeterrondes gebouwd (strakker uitsnijden, AI-belichting, tonale correctie) en alle drie zijn op verzoek teruggedraaid. Zie CLAUDE.md voordat je hier opnieuw aan begint. De varianten staan bewaard in `source-media/images/varianten/`. Masters staan als `Folea-0xx.jpg` in `source-media/images/` (git-ignored).
